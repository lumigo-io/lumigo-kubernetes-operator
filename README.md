...
| [Resource Labels](#opting-in-for-specific-resources) | More granular control - can target specific resources without affecting the entire namespace | Uses default Lumigo project | Separate labels can be used for both logging and tracing | Pods from labeled resources only are mutated to add auto-instrumentation for supported libraries |
