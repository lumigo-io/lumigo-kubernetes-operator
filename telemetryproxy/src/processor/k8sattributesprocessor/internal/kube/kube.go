...
type ClientProvider func(*zap.Logger, k8sconfig.APIConfig, ExtractionRules, Filters, []Association, Excludes, APIClientsetProvider, InformerProvider, InformerProvider, InformerProvider, InformerProviderNamespace) (Client, error)
...
// Deployment represents the identifying data of an apps/v1.Deployment
...
func (d Deployment) cacheKey() string {
	return fmt.Sprintf("%s/%s", d.NamespaceName, d.Name)
}

// Deployment represents the identifying data of an batch/v1.CronJob
type CronJob struct {
	Name          string
	UID           string
	NamespaceName string
...
func (c CronJob) cacheKey() string {
	return fmt.Sprintf("%s/%s", c.NamespaceName, c.Name)
}
